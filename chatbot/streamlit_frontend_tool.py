...
if "chat_threads" not in st.session_state:
    st.session_state["chat_threads"] = retrieve_all_threads()  # dict: {thread_id: title}
...
for thread_id in list(st.session_state["chat_threads"].keys())[::-1]:
    title = st.session_state["chat_threads"][thread_id]
    if st.sidebar.button(title, key=str(thread_id)):
        ...
        for msg in messages:
            if isinstance(msg, ToolMessage):
                continue
            if isinstance(msg, AIMessage) and (not msg.content or msg.tool_calls):
                continue
